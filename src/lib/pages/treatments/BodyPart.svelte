<script lang="ts">
	...
	import { ArrowUp, ArrowDown, Check, X, PersonStanding } from 'lucide-svelte';
...
<Select.Root
	type="multiple"
	value={t.sel_body_parts}
	onValueChange={(e) => {
		t.updateSelectedBodyParts(e);
		t.filterResults();
	}}
>
	<Select.Trigger
	...
	>
		<div class="flex items-center gap-2">
			...
			<span class="font-ui font-medium">Body Part</span>
			{#if t.sel_body_parts && t.sel_body_parts.length > 0}
				<span class="block h-full w-[1px] bg-neutral-200"></span>
				<div class="flex w-max gap-1">
					{#if t.sel_body_parts && t.sel_body_parts.length > 0}
						<button
							on:click|stopPropagation={() => {
								t.clearSelectedBodyParts();
								t.filterResults();
							}}
	...
	<Select.Portal>
		...
		>
			...
			<Select.Viewport class="min-w-[14rem] p-1">
				{#each t.aval_body_parts as part, i (i + part)}
					<Select.Item
					...
					pl-5 text-base capitalize outline-hidden select-none"
						value={part}
						label={returnLocationName(part)} 
					>
						{#snippet children({ selected })}
							<span>
								{returnLocationName(part)} 
							</span>
							{#if selected}
								<div class="ml-auto">
									<Check />
